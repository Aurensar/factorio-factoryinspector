    ...
    for partition,data in pairs(storage.entities) do
        for unit_number, innerData in pairs(data) do
            if unit_number == number_to_delete then
                ...
                count = count + 1
                storage.entities[partition][unit_number] = nil
            end
        end
    end

    for partition,data in pairs(storage.entities_am) do
        for unit_number, innerData in pairs(data) do
            if unit_number == number_to_delete then
                logger.log("[Deep scan] Found entity to remove in AM partition "..partition)
                am_count = am_count + 1
                storage.entities_am[partition][unit_number] = nil
            end
        end
    end

    for partition,data in pairs(storage.entities_md) do
        for unit_number, innerData in pairs(data) do
            if unit_number == number_to_delete then
                logger.log("[Deep scan] Found entity to remove in MD partition "..partition)
                md_count = md_count + 1
                storage.entities_md[partition][unit_number] = nil
            end
        end
    end

    for partition,data in pairs(storage.entities_furnace) do
        for unit_number, innerData in pairs(data) do
            if unit_number == number_to_delete then
                logger.log("[Deep scan] Found entity to remove in F partition "..partition)
                furnace_count = furnace_count + 1
                storage.entities_furnace[partition][unit_number] = nil
            end
        end
    end
...
local function removeEntityByNumber(number)
    ...
    storage.entities[partition][number] = nil
    storage.entities_partition_lookup[number] = nil

    if storage.entities_am_partition_lookup[number] then
        partition = storage.entities_am_partition_lookup[number]
        logger.log("Found entity to remove in assembling machine partition "..partition)
        storage.entities_am[partition][number] = nil
        storage.entities_am_partition_lookup[number] = nil
    end
    if storage.entities_md_partition_lookup[number] then
        partition = storage.entities_md_partition_lookup[number]
        logger.log("Found entity to remove in mining drill partition "..partition)
        storage.entities_md[partition][number] = nil
        storage.entities_md_partition_lookup[number] = nil
    end
    if storage.entities_furnace_partition_lookup[number] then
        partition = storage.entities_furnace_partition_lookup[number]
...
local function enrolNewEntity(entity)
    ...
    if entity.type == "mining-drill" then
        addEntityToPartition(storage.entities_md, storage.md_partition_data, entity, storage.entities_md_partition_lookup, function(e) return { entity=e } end)
    end
...
local function checkEntityBatchForRecipeChanges()
    if storage.entity_count == 0 then
        ...
        for _, surface in pairs(game.surfaces) do
            ...
            for i, entity in ipairs(entities) do
                ...
                storage.entity_count = storage.entity_count + 1
            end
    ...
    if partition_being_checked > storage.global_partition_data.current then
        partition_being_checked = 1
        --logger.log(string.format("Finished checking all %d partitions in %d ticks", storage.global_partition_data.current, (game.tick - update_timer)))
